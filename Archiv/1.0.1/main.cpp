// =====================================================================================
// GarageLuefterregler (code2) - V4.16
// -------------------------------------------------------------------------------------
// Letzte Änderung: 22. Juni 2025, 08:25
...
void sendeLoraDaten(float tIn, float rhIn, float absIn, bool fan) {
  if (millis() - lastSendTime < MIN_SEND_INTERVAL) {
    if (debug2) Serial.println(getTimeStamp() + " [LORA] Senden übersprungen: Zu kurz nach letztem.");
    return;
  }
  ...
  while (digitalRead(LORA_AUX) == LOW) {
    if (millis() - startWait > 1000) {
      if (debug2) Serial.println(getTimeStamp() + " [ERROR] LoRa-Modul ist dauerhaft beschäftigt!");
      return;
    }
    delay(1);
  }
  ...
  // KORREKTUR: sendMessage statt sendFixedMessage verwenden, um zum Normal-Modus des Empfängers zu passen
  ResponseStatus rs = e220ttl.sendMessage(&p, sizeof(p));
  
  if (debug2) {
    if (rs.code == E220_SUCCESS) {
      Serial.printf("%s [LORA] OK ID:%u Ti=%.1f, Hi=%.1f, HiA=%.1f, To=%.1f, Ho=%.1f, HoA=%.1f, Tor=%d, Fan=%s",
        getTimeStamp().c_str(),
        p.messageID,
        p.temperatureInnen, p.humidityInnen, p.absHumidityInnen,
        p.temperatureAussen, p.humidityAussen, p.absHumidityAussen,
        p.torStatus, p.fanOn ? "On" : "Off"
      );
      if (p.fingerEventValid) {
         Serial.printf(", FP: ID=%d, Conf=%d, A-ID=%d\n", p.fingerID, p.confidence, p.actionID);
      } else {
         Serial.println(); // Neue Zeile für normale Nachrichten
      }
    } else {
      Serial.printf("%s [ERROR] LoRa Sendefehler: %s\n", getTimeStamp().c_str(), rs.getResponseDescription().c_str());
    }
  }
...
void sendLoraPeriodically() {
  if (debug1) Serial.println(getTimeStamp() + " [TIMER] Periodisches Senden ausgelöst.");
  readLocalSensors();
...
void updateFan(float innenAbs, float aussenAbs) {
  ...
  if (!fanState && feuchteBedingungErfuellt && letzteTorStatus == 2) {
      ...
      fanState = true;
      if (debug1) Serial.println(getTimeStamp() + " [FAN] Lüfter eingeschaltet (Bedingungen erfüllt).");
  }
...
void onReceive(const uint8_t *mac, const uint8_t *data, int len) {
  ...
  if (len == sizeof(SensorMessage)) {
    ...
    receivedActionID = 0;
    fingerEventReceived = false;
    if (debug1) Serial.printf("%s [ESPNOW] empfangen: To=%.1f, Ho=%.1f, HoA=%.1f, Tor=%d\n", getTimeStamp().c_str(), letzteTempAussen, letzteRH_Aussen, letzteAbsAussen, letzteTorStatus);
    ...
    if (fe.type == 99) {
      ...
      immediateSend = true;
      if (debug1) Serial.printf("%s [ESPNOW] Finger-Event empfangen: ID=%d, Tor=%d\n", getTimeStamp().c_str(), letzteFingerID, letzteTorStatus);
    }
  }
}
...
void setup() {
  ...
  if (cfgContainer.status.code == E220_SUCCESS) {
    ...
    // Setzt die Adresse des EMPFÄNGERS, da dies im Normal-Modus die Zieladresse ist.
    cfg.ADDH = 0;
    cfg.ADDL = 2;
    cfg.CHAN = 23;
    
    // KORREKTUR: Stellt sicher, dass das Modul so konfiguriert ist, dass es RSSI-Daten sendet.
