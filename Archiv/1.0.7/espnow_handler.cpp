...
void init_espnow() {
  ...
  // KORREKTUR: Explizites Hinzufügen des Senders als Peer.
  // Dies kann Kompatibilitätsprobleme bei der Verschlüsselung zwischen
  // unterschiedlichen ESP32-Modellen (wie ESP32 und ESP32-C3) beheben.
  esp_now_peer_info_t peerInfo = {};
  memcpy(peerInfo.peer_addr, known_sender_mac, 6);
  peerInfo.channel = ESPNOW_CHANNEL;
  peerInfo.encrypt = true; // Wichtig: Auch hier die Verschlüsselung erwarten.
...
void onReceive(const uint8_t *mac, const uint8_t *data, int len) {
  ...
  if (len == sizeof(SensorMessage)) {
    ...
    if (DEBUG1) {
        ...
        Serial.printf("  > Typ: %d, Temp: %.1f, Feuchte: %.1f, Tor: %d, Vbat: %.2fV\n", m.type, m.temperature, m.humidity, m.torStatus, m.batteryVoltage);
    }
