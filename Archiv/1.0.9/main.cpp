// =====================================================================================
// GarageLuefterregler (main.cpp) - V5.6 (Sicherheits-Sperre für Relais)
// -------------------------------------------------------------------------------------
// Letzte Änderung: 5. Juli 2025 (Relais-Aktivierung bei inaktivem Link gesperrt)
...
bool espNowLinkActive = false; // Start mit Annahme, dass Link besteht
...
void handle_espnow_watchdog() {
  ...
  if (espNowLinkActive && (millis() - lastEspNowMessageTime > (ESPNOW_TIMEOUT_S * 1000))) {
    ...
    letzteRH_Aussen = 0.0;
    letzteAbsAussen = 0.0;
    letzteTorStatus = 0; // Annahme: Geschlossen ist der sicherste Zustand
...
void setup() {
  ...
  delay(1000);
  if (DEBUG1) {
    Serial.println("\n" + getTimeStamp() + " [SETUP] GarageLuefterregler V5.6 (Modular) startet...");
