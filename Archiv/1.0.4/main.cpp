...
// -------------------------------------------------------------------------------------
// Letzte Änderung: 1. Juli 2025 (Korrektur memcpy-Fehler und Offsets)
...
void onReceive(const uint8_t *mac, const uint8_t *data, int len) {
  ...
  if (len == sizeof(SensorMessage)) {
    SensorMessage m; memcpy(&m, data, sizeof(m)); // KORREKTUR: source ist 'data', nicht 'm'
    ...
    FingerEvent fe;
    memcpy(&fe, data, sizeof(fe)); // KORREKTUR: source ist 'data', nicht 'fe'
    if (fe.type == 99) {
