...
void onReceive(const uint8_t *mac, const uint8_t *data, int len) {
  ...
  // NEU: Bei jedem gültigen Empfang den Zeitstempel aktualisieren und Status prüfen
  lastEspNowMessageTime = millis();
  if (!espNowLinkActive) {
    espNowLinkActive = true;
    if (DEBUG1) Serial.println(getTimeStamp() + " [ESP-NOW] Verbindung zum Sender wiederhergestellt.");
  }
